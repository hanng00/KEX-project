...
class HMN_network:
    ...
    def build(self):
        ...
        print("[BUILD] Network fully built.")

    def _add_background_noise(self):
        background_noise = nest.Create(
            "poisson_generator",
            1,
            {
                "rate": 0,
        ...
        submodule_ids = self.submodule_dict.keys()
        nodes_stimulate = self._get_all_nodes(self.submodule_dict, submodule_ids)

        nest.Connect(background_noise, nodes_stimulate)
    ...
    def simulate(
        ...
        print("[SIMULATE] Configuring devices.. ")
        ...
        print("[SIMULATE] Devices configurated. Simulation started.")
    ...
    def _get_all_nodes(self, submodule_dict, submodule_ids):
        node_collection = nest.NodeCollection()
        for idx in submodule_ids:
            node_collection += submodule_dict[idx]
    ...
    def _generate_base_network(self, nodes, n_modules, E_perc):
        ...
        print(
            f"[BUILD] Generating submodules of size {len_submodule}, total network size {n_of_nodes}"
        )
        ...
        print("[BUILD] Base network configured")
    ...
    def _connect_submodules(self, submodule_dict, P_0, R_ex, E_perc):
        ...
        print("[BUILD] Connecting submodule: ", end="")
        for source_idx, source_nodes in submodule_dict.items():
            print(f"{source_idx}, ", end="")
            if self.verbose:
